...
- **Nix release builds frontend assets from source** — `nix/release.nix` no longer trusts committed minified `priv/static` CSS/JS; its `postBuild` runs an offline `npm ci` (from a prefetched `fetchNpmDeps` cache), the Tailwind v4 CLI, and nixpkgs `esbuild`, then `phx.digest`. So prod styles/scripts are always regenerated from `assets/` — you do **not** need to commit minified `app.css`/`app.js` for a deploy (the committed copies are just dev/watcher artifacts). **Caveat:** if you change `assets/package.json` or `assets/package-lock.json`, you **must** refresh the `npmDeps` hash in `nix/release.nix` (set `hash = lib.fakeHash;`, run `nix build .#pub-quizzer`, copy the `got:` value) — same fixed-output-hash rule as `mix.lock` above.
