defmodule PubQuizzerWeb.QuizJoinController do
  ...
  def join(conn, %{"code" => code}) do
    ...
    cond do
      # Reconnect: already has a team for this event
      get_session(conn, :team_id) && get_session(conn, :event_code) == code ->
        redirect(conn, to: "/quiz/#{code}/lobby")

      true ->
        handle_join(conn, code)
    end
  end
