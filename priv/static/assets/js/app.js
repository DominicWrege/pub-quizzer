...
`
  ...
  var AutoDismiss = {
    mounted() {
      let duration = parseInt(this.el.dataset.duration) || 3e3;
      setTimeout(() => {
  ...
  var OptionSorter = {
    ...
    _resolveTarget(rows, cy) {
      ...
      rows.forEach((r, i) => {
        ...
        const dist = Math.abs(cy - (rect2.top + rect2.height / 2));
        if (dist < minDist) {
          minDist = dist;
          closestIdx = i;
        }
    ...
    _reorder(from, to) {
      ...
      const fromIdx = from;
      const oldCorrect = parseInt((_a = this.el.dataset.correctIndex) != null ? _a : "0");
