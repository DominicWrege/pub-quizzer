...
test.describe("question CRUD", () => {
  test("moderator can create and delete a question", async ({ hostPage }) => {
    ...
    await expect(hostPage.locator(`text=${prompt}`)).toBeVisible({ timeout: 10_000 })

    // --- Delete ---
    await hostPage.locator('[phx-click="ask_delete"]').first().click()
    ...
    await expect(hostPage.locator(`text=${prompt}`)).toHaveCount(0, { timeout: 10_000 })
  })
})
